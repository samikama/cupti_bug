...
ARG UID=1000
ARG GID=1000
RUN apt update && apt install -y wget libiberty-dev binutils-dev libunwind-dev sudo gdb cuda-nvcc-11-1 cuda-cupti-dev-11-1 python3.8-minimal python3.8-dev build-essential ca-certificates \
...
COPY . /CuptiBug
RUN cd /CuptiBug && chown -R cuptibug:cuptibug . && python setup.py clean && CUDAToolkit_ROOT=/usr/local/cuda-11.1/ pip install --use-feature=in-tree-build . && pip install psutil
VOLUME ["workspace"]
WORKDIR /workspace
